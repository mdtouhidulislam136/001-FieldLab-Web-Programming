...
export default function MultiOptions({ func, data1, data2, def_time }) {
    ...
    const [tempvals2, setTempvals2] = useState({ max: data2.default_Axis.max, min: data2.default_Axis.min, TickValue: data2.default_Axis.TickValue })
    ...
    */
    ...
    const ChangeData = (type) => {
        ...
        switch (type) {
            ...
            case "axis2":
                func.SetAxis2({ max: Number(tempvals2.max), min: Number(tempvals2.min), TickValue: Number(tempvals2.TickValue) })
                break;

            case "def_axis2":
                setTempvals2({ max: data2.default_Axis.max, min: data2.default_Axis.min, TickValue: data2.default_Axis.TickValue })
                func.SetAxis2({ ...data2.default_Axis })
                break;
    ...
    return (<div className="ChartOptions">
        <div className="y-axis">
                ...
                <div className="contents">
                    ...
                    <button onClick={e => ChangeData("def_axis1")}>DefaultValues</button>
                </div>
            </div>
        </div>
        <div className="y-axis">
               ...
               <div className="sideways">
                <form >
                    Max
                <input type="text" key="max2" value={tempvals2.max} onChange={e => setTempvals2({ ...tempvals2, max: e.target.value })}></input>
                Min
                <input type="text" key="min2" value={tempvals2.min} onChange={e => setTempvals2({ ...tempvals2, min: e.target.value })}></input>
                TickValues
                <input type="text" key="space2" value={tempvals2.TickValue} onChange={e => setTempvals2({ ...tempvals2, TickValue: e.target.value })}></input>
                </form>
                <div className="contents">
                    <button onClick={e => ChangeData("axis2")}>NewValues</button>
                    <button onClick={e => ChangeData("def_axis2")}>DefaultValues</button>
                </div>
            </div>
        </div>
        <div className="time-axis">
            ...
            <div className="selectDiv">
                ...
                <div className="contents">
                    <button onClick={e => ChangeData("time")}>Newtime</button>
